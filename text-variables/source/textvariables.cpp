...
void TextSource::GetStringFormat(Gdiplus::StringFormat& format)
{
...
void TextSource::RemoveNewlinePadding(const Gdiplus::StringFormat& format, Gdiplus::RectF& box)
{
	Gdiplus::RectF before;
	Gdiplus::RectF after;
	Gdiplus::Status stat;

	stat = graphics.MeasureString(L"W", 2, font.get(), Gdiplus::PointF(0.0f, 0.0f),
								  &format, &before);
...
void TextSource::RenderOutlineText(Gdiplus::Graphics& graphics, const Gdiplus::GraphicsPath& path,
								   const Gdiplus::Brush& brush)
{
	DWORD outline_rgba = calc_color(outline_color, outline_opacity);
	Gdiplus::Status stat;

	Gdiplus::Pen pen(Gdiplus::Color(outline_rgba), outline_size);
	stat = pen.SetLineJoin(Gdiplus::LineJoinRound);
...
void TextSource::TransformText()
{
	...
	if (text_transform == S_TRANSFORM_UPPERCASE)
		...
		bool upper = true;
		for (std::wstring::iterator it = text.begin(); it != text.end();
			 ++it) {
...
void clamp(long val, const long& min_val, const long& max_val)
{
	if (val < min_val)
		val = min_val;
	else if (val > max_val)
...
DWORD get_alpha_val(uint32_t opacity)
{
	return ((opacity * 255 / 100) & 0xFF) << 24;
}

DWORD calc_color(uint32_t color, uint32_t opacity)
{
